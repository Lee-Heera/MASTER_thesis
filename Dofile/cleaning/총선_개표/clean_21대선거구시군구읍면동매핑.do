...
*******************************************************************************
* ── 1. 코드 테이블 (2020년 6월 기준) ─────────────────────────────────────────
import excel "$main/Data raw/센서스 공간정보 지역 코드.xlsx", sheet("2020년6월") clear
drop in 1
rename (A B C D E F) (시도코드 시도명칭 시군구코드 시군구명칭 읍면동코드 읍면동명칭)
drop if missing(읍면동명칭)
keep 시도명칭 시군구명칭 읍면동명칭

save "$interim/선거구시군구매핑/code_tbl_2020.dta", replace

* ── 2. 선거 데이터 로드 및 전처리 ─────────────────────────────────────────────
import excel "$main/Data raw/총선_개표/제21대 국회의원선거 개표결과.xlsx", sheet("지역구") clear

rename (A B C D E F) (시도_raw 선거구 읍면동 투표구 선거인수_str 투표수_str)

//ren (G H I J K L M N O P Q R S T) (더불어민주당 미래통합당 우리공화당 민중당 가자평화인권당 공화당 국민혁명배당금당 국민새정당 민중민주당 한나라당 무소속김용덕 유효투표수 무효투표 기권)
* 시도 forward fill
...
//destring 더불어민주당 미래통합당 우리공화당 민중당 가자평화인권당 공화당 국민혁명배당금당 국민새정당 민중민주당 한나라당 무소속김용덕 유효투표수 무효투표 기권, replace ignore(",")

* 수동 매핑: 21대 + 2020년6월 코드테이블 기준 미매칭 없음
gen 시군구 = ""

* 정규화: 선거 데이터 읍면동의 제N동 → N동
gen 읍면동_norm = ustrregexra(읍면동, "제([0-9])", "$1")

gen _id = _n
* 정당별 득표수 포함 전체 컬럼 저장 (xlsx → dta 변환 보존 목적)
save "$interim/선거구시군구매핑/21대총선_읍면동별_총선개표.dta", replace

* ── 3. Pass 1: 원본 읍면동명으로 merge ────────────────────────────────────────
use "$interim/선거구시군구매핑/21대총선_읍면동별_총선개표.dta", clear

rename 시도 시도명칭
rename 읍면동 읍면동명칭
joinby 시도명칭 읍면동명칭 using "$interim/선거구시군구매핑/code_tbl_2020.dta", unmatched(master) _merge(_m1)
rename 읍면동명칭 읍면동
rename 시도명칭 시도
...
preserve
    ...
    replace 시군구 = 시군구명칭
    drop 시군구명칭 _m1
    save "$interim/선거구시군구매핑/21대_matched1.dta", replace
restore
...
joinby 시도명칭 읍면동명칭 using "$interim/선거구시군구매핑/code_tbl_2020.dta", unmatched(master) _merge(_m2)
...
save "$interim/선거구시군구매핑/21대_matched2.dta", replace

* ── 5. 합치기 + 중복 해소 ──────────────────────────────────────────────────────
use "$interim/선거구시군구매핑/21대_matched1.dta", clear
append using "$interim/선거구시군구매핑/21대_matched2.dta"
...
* ── 6. 최종 정리 및 저장 ───────────────────────────────────────────────────────
//order 시도 시군구 선거구 읍면동 선거인수 투표수 더불어민주당 미래통합당 우리공화당 민중당 가자평화인권당 공화당 국민혁명배당금당 국민새정당 민중민주당 한나라당 무소속김용덕 유효투표수 무효투표 기권
sort 시도 시군구 선거구 읍면동

count
count if missing(시군구)

save "$data/2020congress_clean.dta", replace
...
preserve
    import excel "$main/Data raw/총선_개표/제21대 국회의원선거 개표결과.xlsx", ///
        sheet("지역구") clear
    rename D 투표구
...
di ""
di "══════════════════════════════════════════"
di "  Robustness Check: 21대 선거구-시군구 매핑"
di "══════════════════════════════════════════"
di ""
di "[ 1 ] 읍면동 행 수"
di "      원본 소계 행 수  : `N_소계원본'"
...
*     PDF 공식 기준 21대 253개이며 데이터도 253개 정상
tempvar sel_tag
...
di "      데이터 선거구 수: `N_선거구'  (PDF 공식 253 → 253 정상)"
if `N_선거구' == 253 {
    di "      → 정상 ✔"
}
else {
...
* 임시파일 정리
erase "$interim/선거구시군구매핑/code_tbl_2020.dta"
// erase "$interim/선거구시군구매핑/21대총선_읍면동별_총선개표.dta"
erase "$interim/선거구시군구매핑/21대_matched1.dta"
erase "$interim/선거구시군구매핑/21대_matched2.dta"
