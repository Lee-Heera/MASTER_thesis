...
**********************************************************************
// 파일 이름과 시도명 매칭 리스트
local in "$main/Data raw/대선_개표/1997"
local out "$interim/대선_개표/1997"
...
forvalues i = 1/`n' {
    ...
    local fname "개표현황[제15대][대통령선거][`region'].xlsx"
...
// 2단계: append로 하나로 합치기
use "$interim/대선_개표/1997/강원.dta", clear

cd "$interim/대선_개표/1997"
local files  `" "강원" "경기" "경남" "경북" "광주" "대구" "대전" "부산" "서울" "울산" "인천" "전남" "전북" "제주" "충남" "충북" "'
...
save "$interim/대선_개표/1997/preappend.dta", replace
*****************************************************************************
** cleaning 
use "$interim/대선_개표/1997/preappend.dta", clear 
...
destring 선거인수 투표수 한나라당 새정치국민회의 국민신당 건설국민승리 공화당 바른정치연합 통일한국당 유효투표수 무효투표 기권, replace ignore(",")
******************* 시군구 단위 아닌것 확인 *********************
* ~갑/~을로 끝나는 변수들 -> 원래 ~시/군/구로 끝나게 
gen sigungu_nm_new = sigungu_nm

replace sigungu_nm_new = usubstr(sigungu_nm, 1, ustrlen(sigungu_nm)-1) ///
    if usubstr(sigungu_nm, -1, 1) == "갑" | ///
       usubstr(sigungu_nm, -1, 1) == "을" | ///
       usubstr(sigungu_nm, -1, 1) == "병" | ///
       usubstr(sigungu_nm, -1, 1) == "정"
	  
sort sido_nm sigungu_nm 
**************************************
* 일반 시도에서 ~구까지 내려가는경우 (ex: 부천시 원미구)
gen is_metro = regexm(sido_nm, "광역시|특별시|특별자치시")

gen sigungu_nm_new2 = sigungu_nm_new

replace sigungu_nm_new2 = ustrregexra(sigungu_nm_new, "([가-힣]+시)[가-힣]+구$", "$1") ///
    if is_metro == 0 & regexm(sigungu_nm_new, "[가-힣]+시[가-힣]+구$")

* 확인
list sido_nm sigungu_nm sigungu_nm_new sigungu_nm_new2 ///
    if sigungu_nm_new != sigungu_nm_new2

*--------------------------------------------
* 광역시 이름 정리 
*--------------------------------------------
gen sigungu_nm_new3 = sigungu_nm_new2

replace sigungu_nm_new3 = ustrregexra(sigungu_nm_new2, "([구군])[가-힣]+시$", "$1") ///
    if regexm(sido_nm, "광역시") & regexm(sigungu_nm_new2, "[구군][가-힣]+시$")

* 확인
list sido_nm sigungu_nm_new2 sigungu_nm_new3 ///
    if sigungu_nm_new2 != sigungu_nm_new3

****************************************
* 중구서울시 수동으로 변경 
replace sigungu_nm_new3 = "중구" if sido_nm=="서울특별시" & sigungu_nm=="중구서울시"
****************************************	
collapse (sum) 선거인수 투표수 한나라당 새정치국민회의 국민신당 건설국민승리 공화당 바른정치연합 통일한국당 유효투표수 무효투표 기권, by(sido_nm sigungu_nm_new3)

ren sigungu_nm_new3 sigungu_nm 
**************************************** 
************* 1) 일반 도 소재 + ~시갑,~시을로 쪼개진 경우 
...
*******************************************************************************
* 5) 여수시 통합: 여수시 + 여천군 + 여천시 
*******************************************************************************
replace  sigungu_nm_new ="여수시" if sigungu_nm=="여천군" 
replace  sigungu_nm_new ="여수시" if sigungu_nm=="여천시" 

collapse (sum) 선거인수 투표수 한나라당 새정치국민회의 국민신당 건설국민승리 공화당 바른정치연합 통일한국당 유효투표수 무효투표 기권, by(sido_nm sigungu_nm_new)
...
keep year sido_nm sigungu_nm 선거인수 투표수 한나라당 새정치국민회의 국민신당 건설국민승리 공화당 바른정치연합 통일한국당 유효투표수 무효투표 기권

isid sido_nm sigungu_nm 

save "$data/1997president_clean.dta", replace 
